...
const extensionBlockMatch = pageHtml.match(/const extensions = \[\s*([\s\S]*?)\s*\];\s*let fileFound = false;/);
assert(extensionBlockMatch, 'Expected to find extras media extension groups in the page');

const extensionGroups = Array.from(extensionBlockMatch[1].matchAll(/\[([^\]]+)\]/g), (groupMatch) =>
